...
| `bank/tools/` | `ExportGraph.lean` (a result's solution graph, read from the Lean environment), `gate.sh` (lean4export + `mathesis-adjudicate` over one result), `verify-bank.sh` (every published argument re-derived from its frozen export), `build-site.sh` (build, test and render). |
...
5. Upload `<sha256>.export.gz` to the `bank-exports-v1` release (or a successor), then `bank/tools/build-site.sh` rebuilds `docs/` and runs every check.

## Checks

`bank/tools/build-site.sh` runs the Rust tests, the client typecheck and tests, the no-arbitrary-values lint, the generator, and the prose lint over the rendered tree. CI (`.github/workflows/verify.yml`) re-derives every published argument from its frozen export on the `bank-exports-v1` release (`bank/tools/verify-bank.sh`) and the internal registry from `exports-v1`, runs the same build script, and deploys the rebuilt `docs/` to Pages from `main`.
