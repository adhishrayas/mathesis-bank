-- From leanprover/comparator's char_ofnat_issue test: the kernel expands a string literal
-- through `Char.ofNat` without checking its type, so one returning `Box` is typed `Char`.
prelude
